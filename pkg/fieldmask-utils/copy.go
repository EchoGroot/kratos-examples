...
func StructToMap(filter FieldFilter, src interface{}, dst map[string]interface{}, userOpts ...Option) error {
	...
	for i := 0; i < srcVal.NumField(); i++ {
		...
		case reflect.Struct:
			// time不应该当做结构体转换成map，这里修改了下源码
			// https://github.com/mennanov/fieldmask-utils/issues/27
			srcFieldObj := srcField.Interface()
			if _, ok := srcFieldObj.(time.Time); ok {
				dst[dstName] = srcFieldObj
				continue
			}
